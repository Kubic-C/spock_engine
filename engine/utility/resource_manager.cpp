...
namespace spk {
    ...
    bool font_manager_tt::init() {
        if(!font_map.init(sfk::xor_int_hash<uint32_t>))
            return false;
    ...
    bool font_manager_tt::load_ascii_font(int f_width, int f_height, const char* file_path) {
        uint32_t key = last_used_key++;
        
        if(!font_map.register_key(key))
            return false;

        if(!font_map[key].init()) {
            goto failure;
        } else {
            if(!font_map[key].load_ascii_font(ft_lib, f_width, f_height, file_path)) {
                goto failure;
            } else {
                goto success;
            }
    ...
    bool resource_manager_tt::load_ascii_font(const char* file_path, int f_width, int f_height) {
