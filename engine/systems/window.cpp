...
namespace spk {
    ...
    void window_cs_init(system_ctx_allocater_t& ctx_alloc, flecs::world& world) {
        ...
        comp_window_component_init(world);
        
        world.system<comp_window_t>().ctx(ctx).iter(window_system_update);
        world.system<comp_window_t, tag_current_window_t>().ctx(ctx).interval(state._get_target_fps()).each(window_system_current_update);
    }
}